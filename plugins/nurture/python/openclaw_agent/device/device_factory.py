...
class DeviceFactory:
    ...
    def module(self):
        ...
        if self._module is None:
            if self.device_type == DeviceType.ADB:
                from openclaw_agent.device import adb

                self._module = adb
            elif self.device_type == DeviceType.HDC:
                from openclaw_agent.device import hdc
    ...
    def get_connection_class(self):
        ...
        if self.device_type == DeviceType.ADB:
            from openclaw_agent.device.adb import ADBConnection

            return ADBConnection
        elif self.device_type == DeviceType.HDC:
            from openclaw_agent.device.hdc import HDCConnection
