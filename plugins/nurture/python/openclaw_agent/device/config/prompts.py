...
拟人化操作规范：
- 点击按钮时不要总是点正中心，可以略微偏移到元素范围内的随机位置。
- 滑动操作要模拟真人手指运动轨迹，起点和终点可以有小幅度随机偏移，不要完全固定。
- 操作之间保持自然节奏，不要连续快速操作，也不要过于缓慢机械。
- 当你点击一个元素时，在 thinking 中描述这个元素的大致位置区域（如"屏幕底部中间的按钮"、"页面右上角的图标"）。
